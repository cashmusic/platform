...
if(!defined('STDIN')) {
	...
	foreach ($to_scan as $from_dir => $to_dir) {
		...
		profile_directory($to_dir,$to_dir,$copy_to);
		...
		foreach ($copy_from as $filename => $hash) {
			...
			if ($positive_match) {
				...
				$total_affected++;
				if ($do_copy) {
					if (!is_dir($to_dir.DIRECTORY_SEPARATOR.dirname($filename))) {
						// double-check we have a directory to copy to, if not then make it
						mkdir($to_dir.DIRECTORY_SEPARATOR.dirname($filename),0777,true);
					}
					$success = @copy(
