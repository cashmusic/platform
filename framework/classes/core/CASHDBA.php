	...
	public function getSpecialData($data_name,$conditions=false,$limit=false,$orderby=false) {
		...
		switch ($data_name) {
			...
			case 'CommercePlant_getItemsForUser':
				...
				. "FROM commerce_items i LEFT JOIN assets a ON i.descriptive_asset = a.id "
			...
			case 'CommercePlant_getExternalFulfillmentOrdersByTimestamp':
				$query = "SELECT o.shipping_postal AS postal, t.upc as upc from commerce_external_fulfillment_orders as o "
			...
			case 'CommercePlant_getExternalFulfillmentBackersByJob':
				$query = "SELECT o.name, o.email, o.id, l.uid AS lockcode FROM commerce_external_fulfillment_jobs AS j "
