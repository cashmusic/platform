...
class Google_OAuth2 extends Google_Auth {
  ...
  public function __construct($config = array()) {
    ...
    if (isset($apiConfig) && is_array($apiConfig)) {
      $config = array_merge($apiConfig, $config);;
    }
    
    if (! empty($config['developer_key'])) {
      $this->developerKey = $config['developer_key'];
    }

    if (! empty($config['oauth2_client_id'])) {
      $this->clientId = $config['oauth2_client_id'];
    }

    if (! empty($config['oauth2_client_secret'])) {
      $this->clientSecret = $config['oauth2_client_secret'];
    }

    if (! empty($config['oauth2_redirect_uri'])) {
      $this->redirectUri = $config['oauth2_redirect_uri'];
    }
    
    if (! empty($config['oauth2_access_type'])) {
      $this->accessType = $config['oauth2_access_type'];
    }

    if (! empty($config['oauth2_approval_prompt'])) {
      $this->approvalPrompt = $config['oauth2_approval_prompt'];
    }
  }
